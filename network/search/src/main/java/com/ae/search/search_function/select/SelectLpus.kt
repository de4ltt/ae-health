package com.ae.search.search_function.select
...
internal suspend fun selectLpus(
    ...
    for (lpuElement in lpuElements) {
        val lpu = TypedItemResponse(
            title = lpuElement.selectFirst("span[data-qa=\"lpu_card_heading_lpu_name\"]")!!.text()
                .trim(),
            subtitle = lpuElement.selectFirst("span[data-qa=\"lpu_card_btn_addr_text\"]")!!.text()
                .trim(),
